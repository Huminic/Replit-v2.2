...
| I-236 | **Webhook secrets optional.** VAPI and TextMagic webhook handlers accept all requests if secret env vars not set. `server/routes/webhooks.ts:565-572`, `server/routes/sms.ts:47-55`. Fix: reject webhooks when secret not configured or warn at startup. | BE | OPEN | E |
| I-237 | **Hardcoded password123 fallback in seed.ts:8.** Development guard exists but if NODE_ENV unset, weak passwords seeded. Fix: remove fallback, always require SEED_DEFAULT_PASSWORD or use randomUUID. | BE | OPEN | E |
| I-238 | **Legacy req.body.refreshToken fallback on token refresh.** `server/routes/auth.ts:200` accepts refresh token in body (less secure than httpOnly-only). Fix: remove body fallback. | BE | OPEN | E |
...
| I-243 | **CI deploy health check timeout.** Coolify container doesn't respond to health check within 60s after webhook. Deploy marked as failure even though container starts successfully. Fix: increase timeout or add retry in deploy.yml verify step. | IN | OPEN | M |
