...
Execute real E2E workflow tests across all 4 communication providers (TextMagic, VAPI, Tavus, Resend) with operator authorization, verifying that data flows end-to-end from trigger through provider to Nexxus UI.
...
- Manual E2E verification of each provider with operator authorization
- TextMagic: Send campaign SMS to operator phones, verify delivery
- VAPI: Trigger Elliott->Caroline call, verify transcript and VIN lead creation
- Tavus: Create video session, verify Daily.co room URL
- Resend: Send email notifications, verify in logs
- All tests use real provider APIs (no mocks)
