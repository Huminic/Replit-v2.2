/**
 * widgets.ts — Widget configuration model and landing page definitions
 *
 * Widgets are embeddable customer-facing components that dealers install on their
 * websites. The system supports 4 widget types:
 *  - text: AI-powered text chat (uses backend AI)
 *  - video: Face-to-face video via Tavus AI persona
 *  - voice: Browser-based voice calls via VAPI
 *  - unified: All channels in one FAB (chat, video, voice, SMS, callback)
 *
 * UniversalWidgetSettings controls which channels are enabled across all widgets
 * and is configured in settings.tsx (Tools & Integrations → Universal Widget Settings).
 *
 * Each widget has:
 *  - Appearance config (colors, position, greeting text)
 *  - Targeting rules (audience, pages, devices, timing triggers)
 *  - Allowed domains (CORS/security)
 *  - Embed code snippet (generated by generateWidgetEmbedCode)
 *
 * Landing pages are standalone public pages (e.g., /w/demo) that link to a widget
 * and provide a dedicated customer entry point.
 *
 * PRODUCTION NOTE: Widget JS bundle served from nexxusv2.huminicdev.com/widget/.
 * Analytics (impressions, interactions) tracked via beacon API.
 * Tavus persona IDs and VAPI assistant IDs will come from integration settings.
 */

/** Widget FAB position on the customer's website */
export type WidgetPosition = 'bottom-right' | 'bottom-left';
/** Attention-grabbing animation for the widget FAB button */
export type WidgetAnimation = 'pulse' | 'bounce' | 'none';
/** Widget type — determines the primary communication mode */
...
/** Audience targeting for widget display rules */
export type AudienceType = 'all' | 'leads' | 'returning';

/** Channels available in the unified widget FAB */
export type WidgetChannel = 'chat' | 'video' | 'voice' | 'sms' | 'callback';

/**
 * Universal settings that apply across all widget instances.
 * Configured in settings.tsx → Tools & Integrations section.
 */
...
/**
 * 4 mock widget instances — one per type (text, video, voice, unified).
 * Each has realistic impression/interaction counts for dashboard display.
 */
...
/**
 * 5 mock landing pages — public-facing pages linked to widgets.
 * Rendered by widget-landing.tsx when accessed via /w/:slug route.
 */
...
/** Returns Tailwind bg class for widget status indicator */
...
/**
 * Generates the HTML embed snippet dealers copy-paste into their website.
 * Points to nexxusv2.huminicdev.com production widget JS bundle.
 */
