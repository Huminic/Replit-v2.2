...
export default function InsightsPage({ embedded = false }: { embedded?: boolean }) {
  ...
  const dateParams = `startDate=${startDate}`;
  const orgParam = effectiveOrgId ? `orgId=${effectiveOrgId}&` : '';
  const dashboardUrl = `/api/insights/dashboard?${orgParam}${dateParams}`;
  const reportsUrl = `/api/insights/reports?${orgParam}${dateParams}`;
  const libraryUrl = `/api/insights/library?${orgParam}${dateParams}`;
  ...
  const { data: libraryData, isLoading: libraryLoading } = useQuery<{ metrics: LibraryMetric[]; period: any; campaignSummary: any; appointmentSummary: any }>({
    queryKey: [libraryUrl],
  });
  const libraryMetrics: LibraryMetric[] = libraryData?.metrics || [];
  ...
  const handleExport = (type: string, reportType?: string) => {
    const exportType = reportType || 'pipeline';
    const exportUrl = `/api/reports/export?${orgParam}${dateParams}&type=${exportType}&format=csv`;
    ...
    toast({ title: 'Export Started', description: `Your ${type} report is downloading.` });
  };
            ...
            <Card className="border-amber-500/20 hover-elevate cursor-pointer" onClick={() => setDrillDown('staleLeads')} data-testid="card-stale-leads">
              <CardContent className="p-4">
                <div className="flex items-center gap-3">
                  ...
                  <Button variant="outline" size="sm" onClick={(e) => { e.stopPropagation(); handleExport('CSV', 'pipeline'); }} data-testid="button-export-stale">
  ...
  const renderReports = () => (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="px-4 pt-3 pb-2 border-b border-border flex items-center gap-2 flex-wrap">
        ...
        <Button variant="outline" size="sm" className="ml-auto" onClick={() => handleExport('Report', 'leads')} data-testid="button-export-report-pdf">
    ...
    <>
      <div className="p-4 border-b border-border flex items-center gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-1 ml-auto">
          <Button variant="outline" size="sm" className="text-xs" onClick={() => handleExport('Library', 'leads')} data-testid="button-export-library">
            <Download className="h-3.5 w-3.5 mr-1" /> Export
          </Button>
        ...
        {libraryLoading ? (
          <div className="p-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {Array.from({ length: 8 }).map((_, i) => (
              <Card key={i}><CardContent className="p-4"><Skeleton className="h-4 w-20 mb-3" /><Skeleton className="h-3 w-32 mb-2" /><Skeleton className="h-8 w-24" /></CardContent></Card>
            ))}
          </div>
        ) : (
  ...
  const dateRangePicker = (
    <div className="flex items-center gap-1" data-testid="date-range-picker">
      ...
      <Dialog open={drillDown === 'hotLeads'} onOpenChange={(open) => !open && setDrillDown(null)}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto" data-testid="dialog-hot-leads">
          ...
          <div className="flex justify-end gap-2 mt-2">
            <Button variant="outline" size="sm" onClick={() => handleExport('CSV', 'pipeline')} data-testid="button-export-hot-leads">
      ...
      <Dialog open={drillDown === 'staleLeads'} onOpenChange={(open) => !open && setDrillDown(null)}>
        <DialogContent className="max-w-md" data-testid="dialog-stale-leads">
          ...
          <div className="space-y-3">
            ...
            <Button className="w-full" variant="outline" onClick={() => handleExport('CSV', 'pipeline')} data-testid="button-export-stale-detail">
        ...
        <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto" data-testid="dialog-pipeline-health">
          ...
          <div className="space-y-5">
            ...
            <div className="flex justify-end">
              <Button variant="outline" size="sm" onClick={() => handleExport('Pipeline', 'pipeline')} data-testid="button-export-pipeline">
          ...
          {selectedLibMetric && (
              <>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm"><span className="text-muted-foreground">Current Value</span><span className="text-foreground font-medium">{selectedLibMetric.value}</span></div>
                    <div className="flex justify-between text-sm"><span className="text-muted-foreground">Change</span><span className={cn('font-medium', selectedLibMetric.trend === 'up' ? 'text-green-500' : selectedLibMetric.trend === 'down' ? 'text-red-500' : 'text-muted-foreground')}>{selectedLibMetric.change}</span></div>
                    <div className="flex justify-between text-sm"><span className="text-muted-foreground">Trend</span><span className="text-foreground font-medium capitalize">{selectedLibMetric.trend}</span></div>
                  </div>
                  <div className="flex gap-2 justify-end">
