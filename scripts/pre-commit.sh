...
# ═══════════════════════════════════════════════════════════════════
# GATE 1.7: Context freshness (auto-generated by build-context.sh)
# ═══════════════════════════════════════════════════════════════════
echo "[Gate 1.7/7] Context freshness..."

CONTEXT_FILE="$HOME/.claude/projects/-home-ubuntu-Claude-store-nexxus2-2-replit/memory/context.md"
if [ -f "$CONTEXT_FILE" ]; then
  # Check that context.md references the current sprint
  CONTEXT_SPRINT=$(grep -oE '[A-Z]+-[0-9]+' "$CONTEXT_FILE" | sort -u | tail -1)
  if [ -n "$CONTEXT_SPRINT" ]; then
    echo "  PASS (context references $CONTEXT_SPRINT, committing $SPRINT)"
  else
    echo "  SKIP (no sprint reference found in context.md)"
  fi
else
  echo "  SKIP (context.md not found — run /refresh-context)"
fi
