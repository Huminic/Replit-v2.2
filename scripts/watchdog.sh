...
SESSION_STATE="$HOME/.claude/projects/-home-ubuntu-Claude-store-nexxus2-2-replit/memory/context.md"
...
# ─────────────────────────────────────────────
# C7: Context freshness (auto-generated by build-context.sh)
# ─────────────────────────────────────────────
check_c7() {
  local result="PASS"
  local details=""
  ...
  if [ ! -f "$SESSION_STATE" ]; then
    out "C7  context freshness:   VIOLATION — context.md does not exist"
    VIOLATION_COUNT=$((VIOLATION_COUNT + 1))
    return
  fi
  ...
  if [ "$age_hours" -ge 8 ]; then
    result="VIOLATION"
    details="context.md is ${age_hours}h old (max 8h — rebuilt on session start)"
  fi

  if [ "$result" = "VIOLATION" ]; then
    out "C7  context freshness:   VIOLATION — $details"
    VIOLATION_COUNT=$((VIOLATION_COUNT + 1))
  else
    out "C7  context freshness:   PASS (${age_hours}h old)"
    PASS_COUNT=$((PASS_COUNT + 1))
  fi
}
...
check_c15() {
  ...
  # Check context.md freshness vs last commit timestamp
  # context.md is auto-generated by build-context.sh on SessionStart
  if [ -f "$SESSION_STATE" ]; then
    local last_commit_ts
    last_commit_ts=$(cd "$PROJECT_ROOT" && git log -1 --format="%ct" 2>/dev/null || echo "0")
    local context_mtime
    context_mtime=$(stat -c %Y "$SESSION_STATE")
    # context.md updated more than 8 hours before last commit = stale (generous threshold for auto-generated file)
    if [ $((last_commit_ts - context_mtime)) -gt 28800 ] && [ "$last_commit_ts" -gt 0 ]; then
      result="VIOLATION"
      details+="context.md last updated $(( (last_commit_ts - context_mtime) / 3600 ))h before latest commit; "
    fi
  fi
